...
type LoginCountRow = {
  ...
  userNo: string;
  userNm: string | null;
  departmentNm: string | null;
  positionNm: string | null;
  jobNm: string | null;
  loginCount: number;
};

type LoginCountFilters = { dateFrom: string; dateTo: string; keyword: string };

async function searchLoginCounts(
  page: number,
  size: number,
  filters: LoginCountFilters,
): Promise<{ items: LoginCountRow[]; totalCount: number }> {
  const res = await fetch(`${API_BASE}/api/v1/admin/login-history/count/search`, {
    method: "POST",
  ...
  return {
    items: (json.data?.list ?? []) as LoginCountRow[],
    totalCount: json.data?.totalCount ?? 0,
  };
}

export default function LoginCountPage() {
  ...
  const [data, setData] = useState<LoginCountRow[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    searchLoginCounts(page, pageSize, {
  ...
  const handleExcelDownload = async () => {
    try {
      ...
      exportToExcel(all, {
        filenamePrefix: "로그인 카운트 로그",
        headers: ["NO", "UserNo", "이름", "부서", "직급", "직책", "로그인 횟수"],
        mapRow: (r) => [
          r.rowNum, r.userNo, r.userNm ?? "-", r.departmentNm ?? "-",
          r.positionNm ?? "-", r.jobNm ?? "-", r.loginCount,
        ],
      });
    } catch (e) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)]">
      <div className="mb-5 shrink-0">
        <h2 className="text-lg font-bold">로그인 카운트</h2>
        <p className="text-xs text-gray-500 mt-1">
          사용자별·기간별 로그인 횟수 집계를 확인하는 화면입니다.
        </p>
      </div>
        ...
        <table className="w-full min-w-[900px]">
          <thead className="sticky top-0 z-10">
            <tr className="text-[11px] text-gray-500 border-b border-gray-200 bg-gray-50 text-center">
              ...
              <th className="px-3 py-2 font-medium">UserNo</th>
              ...
              <th className="px-3 py-2 font-medium">로그인 횟수</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              data.map((r) => (
                ...
                >
                  <td className="px-3 py-1.5 text-gray-400 font-mono">{r.rowNum}</td>
                  <td className="px-3 py-1.5 text-gray-400 font-mono">{r.userNo}</td>
                  <td className="px-3 py-1.5 font-medium text-gray-900">{r.userNm ?? "-"}</td>
                  <td className="px-3 py-1.5 text-gray-600">{r.departmentNm ?? "-"}</td>
                  <td className="px-3 py-1.5 text-gray-600">{r.positionNm ?? "-"}</td>
                  <td className="px-3 py-1.5 text-gray-500">{r.jobNm ?? "-"}</td>
                  <td className="px-3 py-1.5 font-semibold text-orange-600 font-mono">{r.loginCount.toLocaleString()}</td>
                </tr>
              ))
            )}
          </tbody>
